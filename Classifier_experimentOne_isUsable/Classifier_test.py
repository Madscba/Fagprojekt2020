...
class classifier_validation():
    ...
    def test(self,type,folds=None,classifyers=["SVM", "LDA", "DecisionTree","GNB", "RF"],logname="test.json"):
        """
    ...
    def two_layes(self,type,classifyers=["SVM", "LDA", "DecisionTree", "RF"],EXP_name=None):
        ...
        for n in range(Nfold):
            ...
            Outerresult=self.test(folds=testdict,classifyers=[best], type=type, logname=None)
            Gen_error.iloc[n] = Egen_s

            Best_log.loc[n,"Best"]=best
            Best_log.loc[n,"Best_AC"]=Outerresult.iloc[0,0]  #Insert best value
            Best_log.iloc[n, 2:5] = Outerresult.iloc[0, 1:4] #Update meta data
