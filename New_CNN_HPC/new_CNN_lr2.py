...
np.save(('train_acc_l2_1.npy'), train_acc_data)
print("\n reached: second saving place")
train_loss_data = np.asarray(train_loss)
np.save(('train_loss_l2_1.npy'), train_loss_data)
valid_acc_data = np.asarray(val_acc)
np.save(('valid_acc_l2_1.npy'), valid_acc_data)
valid_loss_data = np.asarray(val_loss)
np.save(('valid_loss_l2_1.npy'), valid_loss_data)
wrong_guesses_data = np.asarray(wrong_guesses)
np.save(('wrong_guesses_l2_1.npy'), wrong_guesses_data)
wrong_pred_data = np.asarray(wrong_predictions)
np.save(('wrong_guesses_class_l2_1.npy'), wrong_pred_data)