...
for path, q in fileName_quality:
    ...
    for low, up in zip(lower_bound,upper_bound):
        ...
        #plt.yscale('log')
        plt.axis('off')
        pxx, freqs, bins, im = plt.specgram(data, Fs = freq,noverlap =200,NFFT = 201,scale= 'dB',cmap='gray')
        pxx = np.log1p(pxx)
        #plt.show()
    ...
    #save spectrogram into correct folder
        if q == 1: # Tjek if current data is recorded on male
             ...
             if test_or_train < 0.9:
                 plt.savefig(r'C:\Users\Mads-_uop20qq\Documents\fagprojekt\Fagprojekt2020\testSpektrograms\test'+Img_name, edgecolor = None, bbox_inches='tight',pad_inches = 0)
