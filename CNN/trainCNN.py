...
def test_CNN(model,X_train,y_train,X_valid,y_valid,batch_size):
    ...
    for i in range(num_batches):
        ...
        batch_image = np.zeros((10,224,224))
        for j in range(batch_size):
            image_resized = resize(Xtrain[idx[j]], (224, 224), anti_aliasing=True)
    ...
    for i in range(num_test_batches):
        ...
        batch_image = np.zeros((batch_size,224,224))
        for j in range(batch_size):
        ...
        preds = np.argmax(output.data.numpy(), axis=-1)
        correct += np.sum(y_batch_v.data.numpy() == preds)
        print(i, " Now validation")

    val_acc = correct / float(num_test_samples)
